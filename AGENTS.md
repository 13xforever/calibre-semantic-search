...
- **Store schema** (`store.py`): meta tables versioned via `PRAGMA user_version`; one self-contained module per step in `migrations/`. The heavy chunk-table migration is deferred to `VectorStore.finalize_schema()` (ordered stages `'schema'` → `'backend'` → `'codec'`, see `pending_stages()`), run in a background thread before indexing starts. Every stage is resumable via meta markers (`backend_migrate`, `recompress`) written **before** work begins, so a crash mid-transfer never loses data; orphaned storage from an abandoned switch is swept on open only when no marker exists.
